package com.utc.btl.game_play.validator;

import com.utc.btl.Main;
import com.utc.btl.view_component.Board;
import com.utc.btl.view_component.Cell;
import com.utc.btl.view_component.Piece;
import lombok.Setter;

@Setter
public class Validator3 implements IValidator {

    private Main main;
    private Board board;

    public Validator3(Main main) {
        this.main = main;
    }

    @Override
    public void validate(Cell cell) {
        checkDirection(cell, 0, 1);
        checkDirection(cell, 1, 0);
        checkDirection(cell, 1, 1);
        checkDirection(cell, 1, -1);
        checkDraw();
    }

    /**
     * Kiểm tra theo một hướng với luật chặn xa:
     * Năm quân liên tiếp và không bị chắn bởi đối thủ ở cả hai đầu.
     */
    private void checkDirection(Cell cell, int deltaRow, int deltaCol) {
        Piece target = cell.getPiece();
        int count = 1;

        // Duyệt hướng thuận
        int row = cell.getRow() + deltaRow;
        int col = cell.getCol() + deltaCol;
        while (isValid(row, col) && board.getBoard()[row][col].getPiece().equals(target)) {
            count++;
            row += deltaRow;
            col += deltaCol;
        }
        boolean blockedForward;
        if (isValid(row, col)) {
            Piece p = board.getBoard()[row][col].getPiece();
            blockedForward = !p.isEmpty() && !p.equals(target);
        } else {
            blockedForward = true; // ra ngoài bàn cờ coi như bị chắn
        }

        // Duyệt hướng nghịch
        int row2 = cell.getRow() - deltaRow;
        int col2 = cell.getCol() - deltaCol;
        while (isValid(row2, col2) && board.getBoard()[row2][col2].getPiece().equals(target)) {
            count++;
            row2 -= deltaRow;
            col2 -= deltaCol;
        }
        boolean blockedBackward;
        if (isValid(row2, col2)) {
            Piece p2 = board.getBoard()[row2][col2].getPiece();
            blockedBackward = !p2.isEmpty() && !p2.equals(target);
        } else {
            blockedBackward = true;
        }

        // Nếu đủ 5 và không bị chặn hai đầu thì win
        if (count >= 5 && !(blockedForward && blockedBackward)) {
            main.gamePlayController.win(cell);
        }
    }

    private void checkDraw() {
        int count = 0;
        Cell cell = null;
        for (Cell[] row : board.getBoard()) {
            for (Cell c : row) {
                if (c.getPiece().isEmpty() || c.getPiece().isFocus()) {
                    count++;
                    cell = c;
                    if (count > 0) break;
                }
            }
        }
        if (count == 0) {
            main.gamePlayController.draw(cell);
        }
    }

    private boolean isValid(int row, int col) {
        return row >= 0 && col >= 0
                   && row < board.getBoard().length
                   && col < board.getBoard()[0].length;
    }

    @Override
    public void setBoard(Board board) {
        this.board = board;
    }

    @Override
    public Board getBoard() {
        return null;
    }
}

